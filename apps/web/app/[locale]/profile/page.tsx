...
export default function ProfilePage() {
  ...
  async function updatePersonalInformationWithErrors(formData: FormData) {
    ...
    if (typeof result === "string") {
      ...
      toast.update(toastId, { render: 'Information updated successfully', type: 'success', isLoading: false, autoClose: 3000 })
      router.refresh()
    }
  }
  ...
  return (
    <div className="w-full h-max flex flex-col items-start justify-start gap-4 p-8">
      ...
      <form action={updatePersonalInformationWithErrors} className="w-full h-max grid grid-cols-1 md:grid-cols-2 gap-8">
        <Input
          label="First Name"
          name="firstName"
